...
def eval_forecasting(model, data, train_slice, valid_slice, test_slice, scaler, pred_lens, n_covariate_cols, padding, clustering, validation):
    t = time.time()
...
def forecast_result_to_csv(task, method_name, dataset, path, pred_lens, eval_res, train_time, validation, iter_times):
    result_path = os.path.join(path ,'cost',f"{dataset}_results.csv")
    
    if validation:
        result_path = os.path.join(path ,'cost','validation',f"{dataset}_results_validation.csv")
        
    if iter_times != 1:
        result_path = os.path.join(path, 'cost', 'iter_times', f"{dataset}_results_iter_times_{iter_times}.csv")
    ...
    results['cost_train_time'] = train_time
    value_dict['cost_train_time'] = train_time
