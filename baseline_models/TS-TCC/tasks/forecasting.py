...
def eval_forecasting(model, data, train_slice, valid_slice, test_slice, scaler, pred_lens, n_covariate_cols, clustering):
    padding = 200
...
def forecast_result_to_csv(task, method_name, dataset, path, pred_lens, eval_res):
    result_path = os.path.join(path, task ,f"{dataset}_results_max_scaling.csv")
